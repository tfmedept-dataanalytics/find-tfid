...
Insight terpilih dari lapis 1 muncul di tiga tempat: kartu di halaman **Ringkasan** (4 sinyal
teratas), bagian **Insight catatan ini** di dalam detail tiap catatan, dan kartu **Insight utama**
di menu Analisis (lengkap, diurutkan menurut tingkat perhatian: Perlu tindakan, Perlu perhatian,
Peluang, Catatan metode).

Laporan lapis 1 dapat disalin atau diunduh sebagai Markdown langsung dari menu Analisis,
sehingga bisa dilampirkan ke laporan program tanpa menyalakan AI sama sekali. Kedua lapisan
juga tersedia per catatan: buka satu catatan dari daftar, lalu gunakan bagian
**Analisis catatan** di dalam detailnya.
...
| `ALLOWED_FRAME_ANCESTORS` | tidak | domain yang boleh menyematkan FIND dalam iframe; lihat `EMBED_SHAREPOINT.md` |
| `NEXT_PUBLIC_EMBED_MODE` | tidak | isi `1` bila disematkan di domain lain, agar cookie sesi memakai `SameSite=None` |
...
src/
  ...
  components/
    ...
    InsightList.tsx           penyaji butir insight berperingkat
  ...
  lib/
    ...
    rules.ts                  mesin analisis terstruktur + insight (rule-based, tanpa AI)
