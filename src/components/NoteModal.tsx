...
export function NoteModal({ note, onClose, onAnalyze }: { note: Note; onClose: () => void; onAnalyze?: (n: Note) => void }) {
  const { profile, may, aiEnabled } = useNotes();
    ...
    <div className="ovl" id="modalWrap" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal">
        ...
        <div className="modal-f">
          {may('analyze') && aiEnabled && onAnalyze &&
            <button className="btn btn-g" onClick={() => onAnalyze(note)}>Analisis AI catatan ini</button>}
