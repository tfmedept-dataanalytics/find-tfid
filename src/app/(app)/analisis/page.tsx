...
export default function AnalisisPage() {
  ...
  async function run() {
    ...
    const ctx = [applied.prog && `program ${applied.prog}`, applied.dept && `departemen ${applied.dept}`,
      applied.from && `sejak ${applied.from}`, applied.to && `sampai ${applied.to}`,
      applied.stat === 'submitted' && 'hanya catatan terkirim']
      .filter(Boolean).join(', ') || 'seluruh catatan yang dapat diakses';
    try {
  ...
  return (
    <>
      ...
      <main className="content">
        <div className="card mb"><div className="card-h"><h3>Pilih cakupan analisis</h3>
          ...
          <div className="card-b">
            ...
            <div className="flex fw">
              ...
              <button className="btn btn-g" disabled={!scope.length || busy} onClick={run}>Jalankan sintesis AI</button>
            </div>
          </div></div>
        ...
        <div className="card mt"><div className="card-h"><h3>Hasil analisis</h3></div>
          <div className="card-b">
            {busy ? (
              ...
              <div className="empty"><div className="e-t">Belum ada analisis dijalankan</div>
                <div className="e-d">Tetapkan cakupan di atas, lalu jalankan sintesis. Ringkasan statistik di atas dihitung tanpa AI dan selalu tersedia.</div></div>
            )}
          </div></div>
      </main>
    </>
